...
func (s String) Interpolate(resolver Resolver) (string, error) {
	if interpolationAnchoredRegex.MatchString(string(s)) {
		var dst string
		if err := parseVar(string(s)).InterpolateInto(resolver, &dst); err != nil {
			return "", err
		}
		return dst, nil
	}
	...
	interpolated := interpolationRegex.ReplaceAllStringFunc(string(s), func(name string) string {
		var val interface{}
		if err := parseVar(name).InterpolateInto(resolver, &val); err != nil {
...
func ParseVar(s string) (Var, bool) {
	if !interpolationAnchoredRegex.MatchString(s) {
		return Var{}, false
	}
	return parseVar(s), true
}

func parseVar(name string) Var {
...
func interpolate(node interface{}, resolver Resolver) (interface{}, error) {
	...
	case string:
		if interpolationAnchoredRegex.MatchString(typedNode) {
			var dst interface{}
			err := parseVar(typedNode).InterpolateInto(resolver, &dst)
			if err != nil {
				return nil, err
			}
			return dst, nil
