...
func (visitor *planVisitor) VisitTask(step *atc.TaskStep) error {
	visitor.plan = visitor.planFactory.NewPlan(atc.TaskPlanSkeleton{
		Name:              step.Name,
...
func (visitor *planVisitor) VisitGet(step *atc.GetStep) error {
	...
	visitor.plan = visitor.planFactory.NewPlan(atc.GetPlanSkeleton{
		Name: step.Name,
...
func (visitor *planVisitor) VisitPut(step *atc.PutStep) error {
	...
	atcPutPlan := atc.PutPlanSkeleton{
		Type:     resource.Type,
		Name:     logicalName,
		Resource: resourceName,
		Source:   resource.Source,
	...
	dependentGetPlan := visitor.planFactory.NewPlan(atc.GetPlanSkeleton{
		Type:        resource.Type,
		Name:        logicalName,
		Resource:    resourceName,
	...
	visitor.plan = visitor.planFactory.NewPlan(atc.OnSuccessPlanSkeleton{
		Step: putPlan,
...
func (visitor *planVisitor) VisitInParallel(step *atc.InParallelStep) error {
	...
	visitor.plan = visitor.planFactory.NewPlan(atc.InParallelPlanSkeleton{
		Steps:    steps,
...
func (visitor *planVisitor) VisitAcross(step *atc.AcrossStep) error {
	...
	acrossPlan := atc.AcrossPlanSkeleton{
		Vars:     vars,
	...
	for _, vals := range cartesianProduct(step.Vars) {
		...
		acrossPlan.Steps = append(acrossPlan.Steps, atc.VarScopedPlanSkeleton{
			Step:   visitor.plan,
			Values: vals,
		})
	}
...
func (visitor *planVisitor) VisitSetPipeline(step *atc.SetPipelineStep) error {
	visitor.plan = visitor.planFactory.NewPlan(atc.SetPipelinePlanSkeleton{
		Name:     step.Name,
		File:     step.File,
		Team:     step.Team,
		Vars:     step.Vars,
...
func (visitor *planVisitor) VisitLoadVar(step *atc.LoadVarStep) error {
	visitor.plan = visitor.planFactory.NewPlan(atc.LoadVarPlanSkeleton{
		Name:   step.Name,
		File:   step.File,
		Format: step.Format,
		Reveal: step.Reveal,
...
func (visitor *planVisitor) VisitTry(step *atc.TryStep) error {
	...
	visitor.plan = visitor.planFactory.NewPlan(atc.TryPlanSkeleton{
		Step: visitor.plan,
	})
...
func (visitor *planVisitor) VisitTimeout(step *atc.TimeoutStep) error {
	...
	visitor.plan = visitor.planFactory.NewPlan(atc.TimeoutPlanSkeleton{
...
func (visitor *planVisitor) VisitRetry(step *atc.RetryStep) error {
	retryStep := make(atc.RetryPlanSkeleton, step.Attempts)
...
func (visitor *planVisitor) VisitOnSuccess(step *atc.OnSuccessStep) error {
	plan := atc.OnSuccessPlanSkeleton{}
...
func (visitor *planVisitor) VisitOnFailure(step *atc.OnFailureStep) error {
	plan := atc.OnFailurePlanSkeleton{}
